(() => {
  ...
  setText("[data-teaching-summary]", p.teachingSummary);
  setText("[data-contact-intro]", `Based at ${p.affiliation}. ${p.availability}`);
  setText("[data-year]", new Date().getFullYear());


  const profileImage = $("[data-profile-image]");
  if (profileImage && p.portrait) {
    const portraitUrl = safeUrl(p.portrait);
    if (portraitUrl) profileImage.src = portraitUrl;
    if (p.portraitAlt) profileImage.alt = p.portraitAlt;
  }
  ...
  const teachingRoot = $("[data-teaching]");
  if (teachingRoot && Array.isArray(data.teaching)) {
    teachingRoot.innerHTML = data.teaching.map((item) => `
      <article class="teaching-card">
        <p class="teaching-role">${escapeHtml(item.role)}</p>
